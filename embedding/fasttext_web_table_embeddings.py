...
class FastTextWebTableModel:
    ...
    def _clear_string(self, text_value):
        result = ''
        for c in text_value:
            if c in utils.VALID_CHARS_SET_WITH_SPECIAL_SINGS:
                result += c
        return utils.RE_REGULARIZE_NUMBERS.sub('@', utils.RE_REGULARIZE_SPECIAL_SIGNS.sub('*', result.replace(' ', '_'))).lower()
